...
- **请求边界收紧（G10.16）**：① `chat/stream` 的 `query` 与 `unified-chat` 的 `message`
  ...
  实际大小校验仍是最终防线（预检头可伪造）。3 个回归测试。
- **非流式统一入口消息落库（G10.15）**：`POST /api/v1/unified-chat/` 此前从不持久化
  用户与助手消息——多轮记忆丢该轮、会话不进线程列表、反馈无目标。修复后与流式端点
  对齐：加载历史后先落库用户消息，回答成功后落库助手消息（仅成功路径），多 Agent
  统一入口与快路径 `/chat/stream` 的消息行为一致。1 个回归测试。
- **重复上传不再留下孤儿文件（G10.12）**：上传流程改为**先查重后写盘**——sha256 查重前置到
  落盘之前，重复上传（同用户同内容不同文件名，或跨用户 409 路径）直接短路返回，不再"先写
  source/ 再发现重复"；并发双传竞态仍由 `INSERT OR IGNORE` 原子裁决，落败方删除自己刚写的
  副本（仅当路径与权威记录不同才删，同路径文件已被引用不能删）。此前每次同内容不同名重复
  上传都会在 `data/source/` 累积一个未被任何记录引用的孤儿文件。2 个回归测试。
...
- **登录锁定按来源 IP 作用域（G10.13）**：登录失败锁定的 username 维度 key 从全局
  ...
  现锁定仅作用于 (来源 IP, 账户) 组合，跨 IP 撞库仍由 `ip:{ip}` 维度兜底（单 IP 5 次
  失败即锁）。与 `LoginThrottle` 文档声明对齐，1 个回归测试。
- **密码哈希迭代数对齐 OWASP（G10.12）**：PBKDF2-HMAC-SHA256 迭代数 100_000 → 600_000
  （OWASP 2023 密码存储建议下限）。迭代数随哈希串存储（`pbkdf2$<iter>$<salt>$<dk>`），
  校验时按存储值读取——提升仅影响新哈希，存量密码无需重登即可继续校验，未来可继续提升。
